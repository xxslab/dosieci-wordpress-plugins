...
final class HistoryPage {
	...
	public function render(): void {
		if ( isset( $_POST['dosieci_clear_history'] ) ) {
			...
			echo '<div class="notice notice-success is-dismissible"><p>' . esc_html__( 'Historia wyczyszczona.', 'dosieci-ai-operator' ) . '</p></div>';
		}

		$conversation = AjaxController::historyForCurrentUser();
		?>
		<div class="wrap dosieci-ai">
			<h1><?php esc_html_e( 'Historia rozmów', 'dosieci-ai-operator' ); ?></h1>

			<?php if ( array() === $conversation ) : ?>
				<p><?php esc_html_e( 'Brak zapisanej historii.', 'dosieci-ai-operator' ); ?></p>
			<?php else : ?>
				<div class="dosieci-ai-chat-log">
					<?php foreach ( $conversation as $turn ) : ?>
						...
						if ( 'assistant_tool_use' === $role ) {
							$label   = __( 'Narzędzie (żądanie)', 'dosieci-ai-operator' );
							$content = (string) ( $turn['tool_name'] ?? '' );
						} elseif ( 'tool_result' === $role ) {
							$label = __( 'Narzędzie (wynik)', 'dosieci-ai-operator' );
						} elseif ( 'assistant' === $role ) {
							$label = __( 'Operator', 'dosieci-ai-operator' );
						} else {
							$label = __( 'Ty', 'dosieci-ai-operator' );
						}
						?>
				...
				<form method="post">
					...
					<button type="submit" name="dosieci_clear_history" value="1" class="button">
						<?php esc_html_e( 'Wyczyść moją historię', 'dosieci-ai-operator' ); ?>
					</button>
				</form>
			<?php endif; ?>
		</div>
